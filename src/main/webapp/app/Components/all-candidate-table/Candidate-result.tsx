...
import PaginatedTable from '../../Components/paginated-table/table-result';
...
import {
  ...
  downloadCandidateResultSummary,
...
export interface IAllCandidateResultProp extends prop, StateProps, DispatchProps {}

const AllCandidateResult = (props: IAllCandidateResultProp) => {
  const {
    candidateList: [candidates, count],
    pageStatus,
    candidateId,
    candidatesResultDownloads,
    candidatesResultSummaryDownloads,
  } = props;
  ...
  const handleExportSummary = () => {
    props.downloadCandidateResultSummary(startDate, endDate);
    setOpenExportSummary(false);
  };

  useEffect(() => {
    if (candidatesResultDownloads !== null) {
      exportToCSV(candidatesResultDownloads, `CandidatesResult-${dayjs(Date.now()).format('YYYY-MM-DD')}`);
      props.resetDownload();
    }
    if (candidatesResultSummaryDownloads !== null) {
      exportToCSV(candidatesResultSummaryDownloads, `CandidatesResultSummary-${dayjs(Date.now()).format('YYYY-MM-DD')}`);
      props.resetDownload();
    }
  }, [candidatesResultDownloads, candidatesResultSummaryDownloads]);
...
const mapStateToProps = ({ authentication, candidate, candidateDetail }) => ({
  ...
  candidatesResultDownloads: candidate.candidatesResultDownloads,
  candidatesResultSummaryDownloads: candidate.candidatesResultSummaryDownloads,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(AllCandidateResult);
