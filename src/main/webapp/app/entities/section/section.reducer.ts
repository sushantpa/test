...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_SECTION: 'section/PARTIAL_UPDATE_SECTION',
...
export default (state: SectionState = initialState, action): SectionState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_SECTION):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_SECTION):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_SECTION):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<ISection> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_SECTION,
